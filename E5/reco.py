...
class Hit:
    ...
    def pos_module(self, other): 
        """ ordina 2 hit time in base al modulo dal quale sono stati rilevati """
        ord = False
        if(self.id_module<other.id_module or self.id_module==other.id_module):
            ord=True
        return ord
        
    def pos_detector(self, other): 
        """ ordina 2 hit time in base al sensore dal quale sono stati rivelati 
            (se sono relativi a 2 moduli diversi, prevale il numero del modulo) """
        ord=False
    ...
    def ord_module(self, other): 
        """ ordina 2 hit time UGUALI in base al modulo dal quale sono stati rivelati. """
        ord = False
        if (self.hit_time_rev == other.hit_time_rev):
            ord= self.pos_module(self, other)
        return ord
        
    def ord_detector(self, other): 
        """ ordina 2 hit time UGUALI in base al sensore dal quale sono stati rilevati
            (se sono relativi a due moduli diversi, prevale il numero del modulo) """
        ord= False
        if (self.hit_time_rev == other.hit_time_rev):
            if(self.id_module == other.id_module):
                ord = self.id_detector < other.id_detector
