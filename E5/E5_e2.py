...
(ax1,ax2), (ax3, ax4) = gs.subplots(sharex=True)
plt.suptitle(r'$\Delta$ Hit_time histogram for the different detectors ', fontsize=13, color = 'red')
...
ax3.set_title('M2', color='navy', fontsize=10)
ax3.set_ylabel('hits', fontsize=8)
ax3.set_xlabel(r'$Log(\Delta$ hit_time) (ns)', fontsize=8)
ax3.tick_params(axis='x', size=5)
ax3.tick_params(axis='y',size=5)
...
ax4.set_title('M3', color='navy', fontsize=10)
ax4.set_ylabel('hits', fontsize=8)
ax4.set_xlabel(r'$Log(\Delta $hit_time) (ns)', fontsize=8)
...
fig1.savefig('hit_time hist.png')
fig2.savefig('delta_hit_time hist.png')

'''
INTERPRETAZIONE DEI GRAFICI:
Gli istogrammi degli hit times per i 4 differenti moduli mostrano che ci sono molti eventi di hit
verso la fine dell'acquisizione (in particolare intorno ai 0.7 s) e pochi all'inizio della stessa,
e ciò è vero per tutti e 4 i moduli.

Gli istogrammi della differenza tra due hit time successivi mostrano che la maggior parte degli eventi
hanno un delta_hit molto piccolo (eventi molto vicini tra di loro), 
mentre tutti gli altri presentano una differenza temporale più significativa.
 
In particolare, quasi nessun evento ha una distanza temporale dal successivo compresa tra 40 e 5600 ns e,
poichè gli hit appartenenti ad uno stesso evento saranno presumibilmente raggruppati nel tempo,
mentre hit relativi ad eventi diversi avranno una separazione temporale maggiore,
ciò distingue tra un evento di interesse e un evento di non interesse.

Complessivamente si può evincere che si hanno eventi di interesse verso la fine dell'acquisizione
registrati da tutti e 4 i moduli.
...
plt.savefig('delta_t for all hits.png')
plt.show()


""" Come stabilire la finestra temporale da applicare ai $\Delta t$
    che permetta di raggruppare gli *Hit* dello stesso evento ma separi
    quelli apparteneti ad eventi differenti?
    Pensare a come dovrebbe essere strutturata una eventuale classe Event
    per descrivere l'evento fisico come osservato dal rivelatore (quindi basata suglli Hit). """