...
plt.errorbar(df_fromfile['Julian Date'], df_fromfile['Photon Flux [0.1-100 GeV](photons cm-2 s-1)'], yerr=df_fromfile['Photon Flux Error(photons cm-2 s-1)'], fmt= '.', linewidth=1, color= 'blue')
plt.title('Phothon Flux in function of Julian Date')
plt.xlabel('Julian Date', fontsize=12)
plt.ylabel('Phothon Flux (0.1-100 GeV ph $cm^2 s^{-1}$)', fontsize=12)
plt.yticks(fontsize=10)
plt.xticks(fontsize=10)
plt.savefig('PFlux_JD.png')
plt.savefig('PFlux_JD.pdf')
plt.show()

#Grafico del flusso in funzione del giorno giuliano(asse logaritmico)
plt.errorbar(df_fromfile['Julian Date'], df_fromfile['Photon Flux [0.1-100 GeV](photons cm-2 s-1)'], yerr=df_fromfile['Photon Flux Error(photons cm-2 s-1)'], fmt= '.', linewidth=1, color= 'orange')
plt.title('Phothon Flux in function of Julian Date')
plt.xlabel('Julian Date', fontsize=12)
plt.ylabel('log(Phothon Flux)(0.1-100 GeV ph $cm^2 s^{-1}$)', fontsize=12)
plt.yscale('log')
plt.yticks(fontsize=10)
plt.xticks(fontsize=10)
plt.savefig('log_PFlux_JD.png')
plt.savefig('log_PFlux_JD.pdf')
plt.show()